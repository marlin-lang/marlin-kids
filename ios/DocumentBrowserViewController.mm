...
- (void)documentBrowser:(UIDocumentBrowserViewController *)controller
  ...
  // Set the URL for the new document here. Optionally, you can present a template chooser before
  // calling the importHandler. Make sure the importHandler is always called, even if the user
  // cancels the creation request.
...
- (void)documentBrowser:(UIDocumentBrowserViewController *)controller
  ...
  // Make sure to handle the failed import appropriately, e.g., by presenting an error message to
  // the user.
}
